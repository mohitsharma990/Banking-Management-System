...
int loginCustomer(int connectionFD, int accountNumber, char *password) {
    ...
    /*
    creates or accesses a named semaphore uniquely associated with the given accountNumber. 
    This association is crucial if we want to restrict multiple processes from simultaneously 
    accessing the same account.
    */

    setupSignalHandlers();
    // Proper signal handling is essential when dealing with semaphores to ensure that resources, such as file 
    // descriptors and semaphores, are released correctly even if the program is interrupted. 
    // This prevents resource leaks or deadlocks.

    if (sem_trywait(sema) == -1) {  // is a non-blocking version of sem_wait(), tries to lock the semaphore (1->0), but if fails returns -1 and exits w/o waiting.
        if (errno == EAGAIN) { // returns errono as EAGAIN when not able to acquire the lock.
            printf("Customer with account number %d is already logged in!\n", accountNumber);
        } else {
            perror("sem_trywait failed"); // some other error occured in semaphore
        }
        close(file);
        return 0;
    }
    ...
    //  If credentials are incorrect, releases and unlinks the semaphore to free resources, then closes the file and exits.

    snprintf(semName, 50, "/sem_%d", accountNumber);
    // Semaphore Naming: creates a unique name for the semaphore based on the account number.

    sem_t *sema = sem_open(semName, 0);   // Opening the Semaphore: it assumes the semaphore already exists.
    // Releasing and Cleaning Up:
    if (sema != SEM_FAILED) {
        sem_post(sema); // increments the semaphore, effectively “unlocking” it.
        sem_close(sema);  // closes the semaphore handle.
        sem_unlink(semName); // removes the named semaphore, allowing the system to delete it if no other processes are using it.
    }
